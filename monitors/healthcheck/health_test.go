...
func TestNewHealthcheck(t *testing.T) {
	...
	h, err := NewHealthcheck("local", "http://localhost/health", 5, 0, "", "", MockHealthcheckProvider{Result: atomic.NewInt32(http.StatusOK)}, nil)
...
func TestHealthcheck_DisplayName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c, err := NewHealthcheck(tt.name, tt.url, tt.timeout, tt.respCode, tt.icon, "", nil, nil)
			if tt.wantErr {
...
func TestHealthcheck_HasRestartContainers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h, err := NewHealthcheck("test", "http://localhost", 5, 0, "", tt.restartContainers, nil, nil)
			assert.NoError(t, err)
...
func TestHealthcheck_Save_WithoutRestartContainers(t *testing.T) {
	h, err := NewHealthcheck("test", "http://localhost", 5, 0, "", "", nil, nil)
	assert.NoError(t, err)
...
func TestHealthcheck_Save(t *testing.T) {
	...
	// Arrange
	c, err := NewHealthcheck("test", "http://localhost/health", 5, 401, "icon-test", "", nil, nil)
	assert.NoError(t, err)
	// Act
	c.Save(cfg)
...
func TestHealthcheck_DefaultImplSmokeTest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c, err := NewHealthcheck("test", ts.URL, 1, tt.expCode, "", "", nil, nil)
			assert.NoError(t, err)
			r := c.Check(t.Context())
