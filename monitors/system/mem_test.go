...
func (m MockMemProvider) Usage() (*mem.VirtualMemoryStat, error) {
	...
	return &mem.VirtualMemoryStat{
		...
		Available:   uint64(m.total - m.usage.Load()*m.total),
		Used:        uint64(m.usage.Load() * m.total),
...
func TestNewMem(t *testing.T) {
	...
	d := NewMem(90, "", MockMemProvider{usage: atomic.NewFloat64(0)})
...
func TestMem_Check_Mock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewMem(tt.threshold, "", MockMemProvider{usage: atomic.NewFloat64(tt.usage), err: tt.err})
			r := c.Check(t.Context())
